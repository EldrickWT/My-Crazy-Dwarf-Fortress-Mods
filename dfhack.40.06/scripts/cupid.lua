-- Fires Cupid's Arrow at two specified creatures.  Take care when using this on creatures that are already married or dating!

local args = {...}

function cupid (unit_id1, unit_id2)
	...
	if victim1.relations.lover_id ~= -1 then
		print('Warning: the first unit is already dating.  Their courtship will be replaced.')
	end
	if victim2.relations.lover_id ~= -1 then
		print('Warning: the second unit is already dating.  Their courtship will be replaced.')
	end

	print("Beginning Courtship for " .. dfhack.TranslateName(victim1.name,true) .. " and " .. dfhack.TranslateName(victim2.name,true))
	...
	local new_link1 = df.histfig_hf_link_loverst:new()
	local new_link2 = df.histfig_hf_link_loverst:new()
	...
	victim1.relations.lover_id = victim2.id
	victim2.relations.lover_id = victim1.id
end
...
dfhack.with_suspend(cupid, tonumber(args[1]), tonumber(args[2]))
